...
//           <div>
//             <p className="font-semibold text-lg">Top Error Status</p>
...
// File: src/pages/DeviceReportPage.jsx
import axios from "axios";
import { useEffect, useState } from "react";
...
const DeviceReportPage = () => {
  ...
  useEffect(() => {
    const fetchTopErrorPerStatus = async () => {
      try {
        const res = await axios.get("https://server.diwd.cloud/api/v1/getTopErrorPerStatus");
        const data = Array.isArray(res.data?.data) ? res.data.data : [];
        setTopErrorPerStatus(data);
      } catch (err) {
        console.error("Gagal fetch top error per status:", err);
      } finally {
        setLoading(false);
      }
    };

    fetchTopErrorPerStatus();
  }, []);
  ...
  const handleExportAll = async () => {
    ...
    try {
      const endpoints = [
        { url: "getTopHourlyError", section: "TopHourly" },
        { url: "getHourlyErrorTrend", section: "AllHourly" },
        { url: "getTopDailyError", section: "TopDaily" },
        { url: "getDailyStatusTrend", section: "AllDaily" },
        { url: "getTopWeeklyError", section: "TopWeekly" },
        { url: "getWeeklyStatusTrend", section: "AllWeekly" },
        { url: "getTopMothlyError", section: "TopMonthly" },
        { url: "getMonthlyStatusTrend", section: "AllMonthly" },
        { url: "getTopErrorPerStatus", section: "TopErrorPerStatus" },
      ];

      const requests = await Promise.all(
        endpoints.map(({ url }) => axios.get(`https://server.diwd.cloud/api/v1/${url}`))
      );

      const allData = requests.flatMap((res, i) => {
        const data = Array.isArray(res.data?.data) ? res.data.data : [];
        return data.map((r) => ({ section: endpoints[i].section, ...r }));
      });
    ...
    <div className="relative">
      ...
      <div className="p-4 space-y-6 bg-white dark:bg-gray-800 font-Poppins shadow-md rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-10 dark:text-white">
          ...
          <div>
            <div className="mb-2 font-Poppins text-2xl text-center">Status Perangkat</div>
            <PredictStatus />
          </div>
        </div>
      </div>
      <div className="flex justify-end w-full text-sm py-6">
        <button
          ...
          className="border border-blue-600 dark:border-orange-500 dark:text-white rounded-md px-4 py-2 disabled:opacity-50"
        >
          Export
        </button>
      </div>
