...
const ProfileForm = () => {
  ...
  const ProfileField = ({ label, value }) => (
    <div className="text-start m-2">
      <label>{label}</label>
      <div className="border my-3 p-2 rounded-md">{value || "Tidak tersedia"}</div>
    </div>
  );
      ...
      <div className="grid grid-cols-3 gap-4 font-Inter font-light">
        <div>
          <div className="flex flex-col justify-center items-center bg-white rounded-md py-6 px-4">
            <img src={selectedImage || ImgProfile} alt="" className="w-60 h-72" />
            <label htmlFor="inputImage" className="cursor-pointer border border-blue-500 rounded-md w-full h-8 flex justify-center">
              Unggah Foto
              <input id="inputImage" type="file" accept="image/*" onChange={handleImageChange} className="hidden" />
            </label>
          </div>
