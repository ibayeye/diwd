...
const PredictStatus = () => {
    ...
    const fetchStatus = async () => {
      try {
        const response = await fetch(
          "https://server.diwd.cloud/api/v1/getPredictStatus",
          {
            method: "POST",
            headers: {
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg border font-Poppins">
      ...
      {loading ? (
        <div className="flex justify-center items-center space-x-2 text-blue-500 dark:text-orange-500">
          <FaSpinner className="animate-spin text-2xl" />
          <span className="text-sm font-medium">Menganalisis Status Akhir</span>
        </div>
      ) : error ? (
        ...
        <div className="items-center space-x-3 bg-yellow-50 border border-yellow-200 dark:bg-yellow-400 dark:border-none p-4 rounded-md">
          <p className="dark:text-gray-700 text-sm">Status Akhir Alat</p>
          <div className="flex gap-2 mt-2">
            <HiOutlineCheckCircle className="text-yellow-500 dark:text-gray-700 text-2xl" />
            <p className="text-lg font-semibold text-yellow-500 dark:text-gray-700">
              {statusLabel}
            </p>
          </div>
        </div>
